...
def build_model(timesteps: int, num_classes: int):
    model = Sequential([
        SimpleRNN(128, return_sequences=True, input_shape=(timesteps, 1)),
        Dropout(0.3),
        BatchNormalization(),
...
def train_and_evaluate(target: str = 'mood'):
    ...
    model_path = os.path.join(base_dir, f"rnn_{target}.h5")
    model.save(model_path)
    model.save_weights(os.path.join(base_dir, f"rnn_{target}.weights.h5"))
    return metrics
